...
def color_circle(board_value, display_surface, x_coord, y_coord):
    ...
    label = "User: White"
    text_surface, rect = font.render(label, (0, 0, 0))
    text_width, text_height = font.get_rect(label).size
    # font.render(label,True,(0,0,0))
    display_surface.blit(text_surface, (8, 705))
...
def color_destination(display_surface, x_coord_long, y_coord, row, circle_in_a_row, destinations):
    ...
    if [row, circle_in_a_row * 2] == destinations[5]:
        #pg.draw.circle(display_surface, PLAYER2_WHITE, (destinations[5][0]+100,destinations[5][1]+30), CIRCLE_RADIUS, 0)
...
def highlight_move(best_move, display_surface, n):
    [start_x, start_y] = best_move[n]

    circle_start_x, circle_start_y = find_circle_from(start_x, start_y)

    # 调整横坐标偏移量
    if start_x % 2 != 0:
        circle_start_x += int((CIRCLE_DIAMETER + H_SPACING) / 2)
